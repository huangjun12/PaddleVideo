...
def get_norm(bn_norm_type, bn_num_splits):
    ...
    elif bn_norm_type == "batchnorm2d":
        return paddle.nn.BatchNorm2D
    elif bn_norm_type == "sub_batchnorm":
        return partial(SubBatchNorm3D, num_splits=bn_num_splits)
    elif bn_norm_type == "sub_batchnorm2d":
        return partial(SubBatchNorm2D, num_splits=bn_num_splits)
    else:
...
def aggregate_sub_bn_stats(model):
    ...
    for child in model.children():
        if isinstance(child, SubBatchNorm3D) or isinstance(child, SubBatchNorm2D):
...
class SubBatchNorm2D(paddle.nn.Layer):
    """
    Implement based on paddle2.0.
    The standard BN layer computes stats across all examples in a GPU. In some
    cases it is desirable to compute stats across only a subset of examples
    ...
    def __init__(self, num_splits, **args):
        ...
        args["weight_attr"] = False
        args["bias_attr"] = False
        self.bn = paddle.nn.BatchNorm2D(**args)
        # update number of features used in split_bn
        args["num_features"] = self.num_features * self.num_splits
        self.split_bn = paddle.nn.BatchNorm2D(**args)
    ...
    def forward(self, x):
        if self.training:
            n, c, h, w = x.shape
            x = paddle.reshape(
                x, (n // self.num_splits, c * self.num_splits, h, w))
            x = self.split_bn(x)
            x = paddle.reshape(x, (n, c, h, w))
        else:
            x = self.bn(x)
        x = paddle.multiply(x, paddle.reshape(self.weight, (-1, 1, 1)))
        x = paddle.add(x, paddle.reshape(self.bias, (-1, 1, 1)))
        return x
